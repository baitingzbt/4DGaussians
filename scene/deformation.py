...
class Deformation(nn.Module):
    ...
    def create_net(self):
        ...
        self.feature_out2 = [
            nn.Linear(grid_out_dim, self.W, dtype=torch.float32, bias=True)
        ]
        ...
        self.pos_deform = nn.Sequential(
            nn.ReLU(), nn.Linear(self.W + self.extra_point_dim + 3, self.W, dtype=torch.float32, bias=True),
        ...
        self.scales_deform = nn.Sequential(
            nn.ReLU(), nn.Linear(self.W + self.extra_scale_dim + 3, self.W, dtype=torch.float32, bias=True),
        ...
        self.rotations_deform = nn.Sequential(
            nn.ReLU(), nn.Linear(self.W + self.extra_rotation_dim + 3, self.W, dtype=torch.float32, bias=True),
            # nn.ReLU(), nn.Linear(self.W, self.W),
            # nn.ReLU(), nn.Linear(self.W, self.W),
            nn.ReLU(), nn.Linear(self.W, 4, dtype=torch.float32, bias=True)
        )
        self.opacity_deform = nn.Sequential(
            nn.ReLU(), nn.Linear(self.W + self.extra_opacity_dim + 3, self.W, dtype=torch.float32, bias=True),
            # nn.ReLU(), nn.Linear(self.W, self.W),
            # nn.ReLU(), nn.Linear(self.W, self.W),
            nn.ReLU(), nn.Linear(self.W, 1, dtype=torch.float32, bias=True)
        )
        self.shs_deform = nn.Sequential(
            nn.ReLU(), nn.Linear(self.W + 3, self.W, dtype=torch.float32, bias=True),
    ...
    def query_time_force(self, rays_pts_emb, time_emb, force_emb, prev_frames_emb):
        # if force_emb is not None and force_emb.shape[1] != 1:  # NOTE: if use_force = True
    ...
    def forward_dynamic(self, rays_pts_emb, scales_emb, rotations_emb, opacity_emb, shs_emb, time_emb, force_emb, prev_frames):
        # time_emb = time_emb if self.args.use_time else None
        # force_emb = force_emb if self.args.use_force else None
        ...
        if self.args.no_dx:
            ...
            pts = rays_pts_emb[:, :3] * mask + self.pos_deform(
                torch.cat((hidden, rays_pts_emb[:, 3:], time_emb, force_emb[:, :2]), dim=1))
        ...
        if self.args.no_ds:
            ...
            scales = scales_emb[:, :3] * mask + self.scales_deform(
                torch.cat((hidden, scales_emb[:, 3:], time_emb, force_emb[:, :2]), dim=1))
        ...
        if self.args.no_dr:
            ...
            dr = self.rotations_deform(torch.cat((hidden, rotations_emb[:, 4:], time_emb, force_emb[:, :2]), dim=1))
        ...
        if self.args.no_do:
            ...
            opacity = opacity_emb[:, :1] * mask + self.opacity_deform(torch.cat((hidden, time_emb, force_emb[:, :2]), dim=1))

        if self.args.no_dshs:
            shs = shs_emb
        else:
            # DEFAULT
            shs = shs_emb * mask.unsqueeze(-1) + self.shs_deform(
                torch.cat((hidden, time_emb, force_emb[:, :2]), dim=1)).reshape([shs_emb.shape[0], 16, 3])
...
class deform_network(nn.Module):
    def __init__(self, args) :
        ...
        self.register_buffer('force_poc', torch.FloatTensor([(2**i) for i in range(posbase_pe)]))
    ...
    def forward_dynamic(self, points_in, scales_in, rotations_in, opacity_in, shs_in, times_sel_in, force_in, prev_frames):
        ...
        force_emb = force_in # poc_fre(force_in, self.force_poc)
        means3D, scales, rotations, opacity, shs = self.deformation_net.forward_dynamic(
            point_emb, scales_emb, rotations_emb, opacity_in, shs_in, times_sel_in, force_emb, prev_frames
        )
